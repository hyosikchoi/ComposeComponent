    ...
    NavHost(navController = navController, startDestination = "Home", modifier = modifier) {
        composable("Home") {
            Column {
                ...
                Button(onClick = {
                    navController.navigate("Argument/hyosik") {
                        launchSingleTop = true
                    }
                }) {
                    Text(text = "hyosik 으로 이동")
                }
        ...
        composable("Playground") {
            Column {
                ...
                Button(onClick = {
                    ...
                    // 확인해봅니다.
                    /** 스택에서 Home 을 제외한 다른 composable 들을 pop 해서 제거한다. */
                    navController.navigate("Office") {
                        popUpTo("Home") {
                            // 단계 9: `popUpTo`의 후행 람다에 `inclusive = true`를 넣어보고
                            // 스택 이동을 확인해봅시다.
                            /** inclusive = true 로 설정하면 popUpTo 를 설정한 Home 까지 같이 pop 해서 제거합니다. */
                            inclusive = true
                        }
                    }
                }) {
